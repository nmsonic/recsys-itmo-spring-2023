...
class ContextualAndTopPop(Recommender):
    """
    ...
    def __init__(self, tracks_redis, catalog, pop_tracks):
        self.tracks_redis = tracks_redis
        self.fallback = TopPop(tracks_redis, pop_tracks)
        self.catalog = catalog

    def recommend_next(self, user: int, prev_track: int, prev_track_time: float) -> int:
        previous_track = self.tracks_redis.get(prev_track)
        if previous_track is None or prev_track_time < 0.2:
